...
#insertar
    def insertar(self):
        result = Lote.collection.insert_one(self.to_dict())
        self._id = str(result.inserted_id)
        return self._id 
    #modificar
    def actualizar(self):
        Lote.collection.update_one(
            {"_id": ObjectId(self._id)},
            {"$set": self.to_dict()}
        )
    #eliminar
    def eliminar(self):
        Lote.collection.delete_one({"_id": ObjectId(self._id)})
    #obtener todo
    @staticmethod
    def obtener_todos():
        return list(Lote.collection.find())
    #obtener por id 
    @staticmethod
    def obtener_por_id(id): 
        return Lote.collection.find_one({"_id": ObjectId(id)})
#main
